PaGlobal_TutorialPhase_BasicSkill_BladeMaster = {
  _phaseNo = 6,
  _currentType = 0,
  _currentStep = 0,
  _nextStep = 0,
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:checkPossibleForPhaseStart(stepNo)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:checkSkippablePhase()
  if true == self._isSkippable and true == PaGlobal_TutorialManager:checkHaveOverLevelCharacter() then
    return true
  end
  return false
end
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:startPhase(stepNo, typeNo)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:startPhaseXXX(stepNo, typeNo)
  PaGlobal_TutorialManager:setCurrentPhaseNo(self._phaseNo)
  PaGlobal_TutorialManager:setDoingTutorial(true)
  _PA_LOG("\234\179\189\235\175\188\236\154\176", "PaGlobal_TutorialPhase_BasicSkill_BladeMaster:startStep() typeNo : " .. tostring(typeNo))
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:endPhase()
  self._currentStep = 0
  self._nextStep = 1
...
local result = false
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:updatePerFrame(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:handleChangeStep(currentStep)
  if 1 == self._currentType then
    if 1 == self._currentStep then
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:showUiScoldingTooManyKeyInput()
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:changeStepTryBasicSkill()
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:updateTryBasicSkill(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:eventCallUsedSkill_TryBasicSkill(skillWrapper)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:changeStepTrySkillCombo()
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:updateTrySkillCombo(deltaTime)
  PaGlobal_TutorialUiManager:getUiKeyButton():ButtonToggleAll(true)
  if self._totalComboCount <= self._usedComboCount then
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:eventCallUsedSkill_TrySkillCombo(skillWrapper)
  local listNo = 0
  local skillNo = skillWrapper:getSkillNo()
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:changeStepSuggestCallBlackSpirit()
  if 1 == self._currentProgress then
    PaGlobal_TutorialUiManager:getUiBlackSpirit():showSuggestCallSpiritUi()
    PaGlobal_TutorialManager:setAllowCallBlackSpirit(true)
  end
end
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:eventCallAcceptBlackSpiritQuest(isAccept, questGroupNo, questId)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:eventCallAfterBlackSpiritDialogClose()
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:handleEventQuestUpdateNotify(isAccept, questNoRaw)
...
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:handleAfterAndPopFlush()
  if 1 == self._currentType and 3 == self._currentStep then
    self:eventCallAfterBlackSpiritDialogClose()
  end
end
function PaGlobal_TutorialPhase_BasicSkill_BladeMaster:handleEventSelfPlayerUsedSkill(skillWrapper)
