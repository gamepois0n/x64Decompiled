...
function isUseNewQuickSlot()
  if true == ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(CppEnums.GlobalUIOptionType.NewQuickSlot) then
    if true == PaGlobal_TutorialManager:isDoingTutorial() and false == PaGlobal_TutorialManager:isAllowNewQuickSlot() then
      return false
    end
    return true
  end
...
function QuickSlot_Click(slotIndex)
  _PA_LOG("\236\160\149\237\131\156\234\179\164", "slotIndex : " .. tostring(slotIndex))
...
function QuickSlot_UpdatePerFrame(fDeltaTime)
  ...
  for idx, slot in ipairs(self.slots) do
    ...
    if true == PaGlobal_TutorialManager:isDoingTutorial() and 1 == idx and 502 ~= quickSlotInfo._itemKey:getItemKey() then
...
function FGlobal_QuickSlot_CheckDefaultQuickSlotShowAble()
  if true == isUseNewQuickSlot() then
    return false
  elseif false == PaGlobal_TutorialManager:isAllowShowQuickSlot() then
    return false
  end
  return true
end
