...
function AutoState_DialogInteraction:checkChangeState()
  ...
  if true == isQuestComplete() or dialogData:getDialogState() == CppEnums.DialogState.eDialogState_QuestComplete then
    if reward > 0 then
  ...
  if false == isNextButtonShow() and true == newMainQuest then
  ...
  if not newMainQuest and (reward == 0 and bcount ~= 0 or trye == isVisibleDisplayQuestButton() or true == isNextButtonShow()) then
    self._interactionState = AutoDialogInteractionState_Type.ESCAPE
    return
  end
