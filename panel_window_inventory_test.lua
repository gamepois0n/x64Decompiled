...
function inven:init()
  _PA_LOG("\235\176\149\234\183\156\235\130\152", "ExpandSlot Test init 2")
  self.config.slotCount = self.config.slotRows * self.config.slotCols
  self.invenPanelWidth = Panel_Window_Inventory:GetSizeX()
  self.invenPanelHeight = Panel_Window_Inventory:GetSizeY()
  self.scrollAreaWidth = self.cashScrollArea:GetSizeX()
  self.scrollAreaHeight = self.cashScrollArea:GetSizeY()
...
function inven:RefreshPostion()
  for ii = 0, self.config.slotMaxCount - 1 do
    local slotA = self._slotsBackground[ii]
    local row = math.floor(ii / self.config.slotCols)
    local col = ii % self.config.slotCols
    if nil ~= slotA then
      slotA.empty:SetPosX(self.config.slotStartX + self.config.slotGapX * col)
      slotA.empty:SetPosY(self.config.slotStartY + self.config.slotGapY * row)
      slotA.lock:SetPosX(self.config.slotStartX + self.config.slotGapX * col)
      slotA.lock:SetPosY(self.config.slotStartY + self.config.slotGapY * row)
      slotA.empty:SetShow(false)
      slotA.lock:SetShow(false)
    end
    local slotB = self.slots[ii]
    if nil ~= slotB then
      slotB.icon:SetPosX(self.config.slotStartX + self.config.slotGapX * col)
      slotB.icon:SetPosY(self.config.slotStartY + self.config.slotGapY * row)
      slotB.background:SetPosX(slotB.icon:GetPosX())
      slotB.background:SetPosY(slotB.icon:GetPosY())
      slotB.icon:SetEnableDragAndDrop(true)
      slotB.isEmpty = true
      slotB.background:SetShow(false)
      slotB:clearItem()
      slotB.icon:SetEnable(true)
      slotB.icon:SetMonoTone(true)
    end
    local slotC = self.slotEtcData[ii]
    if nil ~= slotC then
      slotC.puzzleControl:SetShow(false)
      slotC.isFirstItem = false
    end
    if ii >= self.config.slotCount then
      slotB.icon:SetIgnore(true)
      slotB.background:SetIgnore(true)
      slotC.puzzleControl:SetIgnore(true)
    else
      slotB.icon:SetIgnore(false)
      slotB.background:SetIgnore(false)
      slotC.puzzleControl:SetIgnore(false)
    end
  end
  local AddSizeX = self.config.slotGapX * (self.config.slotCols - self.config.slotDefaultCount)
  local AddSizeY = self.config.slotGapY * (self.config.slotRows - self.config.slotDefaultCount)
  Panel_Window_Inventory:SetSize(self.invenPanelWidth + AddSizeX, self.invenPanelHeight + AddSizeY)
  self.cashScrollArea:SetSize(self.scrollAreaWidth + AddSizeX, self.scrollAreaHeight + AddSizeY)
  local InitPosX = self.cashScrollArea:GetPosX()
  local InitPosY = self.cashScrollArea:GetPosY() + self.cashScrollArea:GetSizeY() - 1
  local valueX = (self.scrollAreaWidth + AddSizeX) / 4
  btn_AlchemyFigureHead:SetPosX(InitPosX)
  btn_AlchemyFigureHead:SetPosY(InitPosY)
  btn_AlchemyStone:SetPosX(InitPosX + valueX)
  btn_AlchemyStone:SetPosY(InitPosY)
  btn_Manufacture:SetPosX(InitPosX + valueX * 2)
  btn_Manufacture:SetPosY(InitPosY)
  btn_DyePalette:SetPosX(InitPosX + valueX * 3)
  btn_DyePalette:SetPosY(InitPosY)
  self._scroll:SetSize(5, self.cashScrollArea:GetSizeY())
end
  ...
  if CppEnums.ContentsEventType.ContentsType_InventoryBag == itemWrapper:getStaticStatus():get():getContentsEventType() then
    ...
    for index = 0, bagSize - 1 do
      ...
      if nil ~= bagItemWrapper then
        ...
        elseif CppEnums.ItemWhereType.eCashInventory == deleteWhereType then
...
function Inventory_updateSlotData()
  ...
  for ii = 0, self.config.slotCount - 1 do
    ...
    if nil ~= itemWrapper then
      ...
      if (42000 == itemKey or 42001 == itemKey or 41607 == itemKey) and true == PaGlobal_SummonBossTutorial_Manager:isDoingSummonBossTutorial() and not FGlobal_FirstSummonItemUse() then
      ...
      if not isUsableServant and not Panel_Window_Exchange:GetShow() then
        ...
        if slotNo < self.config.slotCount and true == self.slotEtcData[slotNo].isFirstItem and inven.slotEtcData[slotNo].itemKey == itemWrapper:get():getKey():getItemKey() then
  ...
  if isEquip and not Panel_Window_Exchange:GetShow() then
    ...
    if nil ~= equipItemWrapper then
      ...
      elseif enum_class.ClassType_Giant == classType and (29 == equipType or 34 == equipType or isAwakenWeaponContentsOpen and 57 == equipType) or 13 == equipType or 9 == equipType or 11 == equipType or 12 == equipType then
        ...
        equipOffencePoint = (ToClient_getEquipmentItem(itemWrapper:getStaticStatus():getEquipSlotNo()):getStaticStatus():getMinDamage(0) + ToClient_getEquipmentItem(itemWrapper:getStaticStatus():getEquipSlotNo()):getStaticStatus():getMaxDamage(0)) / 2
        matchEquip = true
      elseif enum_class.ClassType_Ranger == classType and (31 == equipType or 32 == equipType or isAwakenWeaponContentsOpen and 57 == equipType) or 13 == equipType or 9 == equipType or 11 == equipType or 12 == equipType then
...
function Inventory_UseItemTargetSelf(whereType, slotNo, equipSlotNo)
  ...
  if (42000 == itemKey or 42001 == itemKey) and PaGlobal_SummonBossTutorial_Manager:isDoingSummonBossTutorial() then
...
function Inventory_CashTabScroll(isUp)
  ...
  local addCount = self.config.slotCols - maxSize % self.config.slotCols
  if addCount == self.config.slotCols then
    addCount = 0
  end
  self.startSlotIndex = UIScroll.ScrollEvent(self._scroll, isUp, self.config.slotRows, maxSize + addCount, self.startSlotIndex, self.config.slotCols)
  Inventory_updateSlotData()
end
...
function TestSlot(cntX, cntY)
  HandleClicked_InventoryWindow_Close()
  local self = inven
  self.config.slotCols = cntX
  self.config.slotRows = cntY
  self.config.slotCount = self.config.slotRows * self.config.slotCols
  self:RefreshPostion()
  Inventory_updateSlotData()
end
