PaGlobal_TutorialPhase_BasicSkill_Combattant = {
  _phaseNo = 6,
  _currentType = 0,
  _currentStep = 0,
  _nextStep = 0,
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:checkPossibleForPhaseStart(stepNo)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:checkSkippablePhase()
  if true == self._isSkippable and true == PaGlobal_TutorialManager:checkHaveOverLevelCharacter() then
    return true
  end
  return false
end
function PaGlobal_TutorialPhase_BasicSkill_Combattant:startPhase(stepNo, typeNo)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:startPhaseXXX(stepNo, typeNo)
  PaGlobal_TutorialManager:setCurrentPhaseNo(self._phaseNo)
  PaGlobal_TutorialManager:setDoingTutorial(true)
  _PA_LOG("\234\179\189\235\175\188\236\154\176", "PaGlobal_TutorialPhase_BasicSkill_Combattant:startStep() typeNo : " .. tostring(typeNo))
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:endPhase()
  self._currentStep = 0
  self._nextStep = 1
...
local result = false
function PaGlobal_TutorialPhase_BasicSkill_Combattant:updatePerFrame(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:handleChangeStep(currentStep)
  if 1 == self._currentType then
    if 1 == self._currentStep then
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:showUiScoldingTooManyKeyInput()
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:changeStepTryBasicSkill()
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:updateTryBasicSkill(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:eventCallUsedSkill_TryBasicSkill(skillWrapper)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:changeStepTrySkillCombo()
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:updateTrySkillCombo(deltaTime)
  PaGlobal_TutorialUiManager:getUiKeyButton():ButtonToggleAll(true)
  if self._totalComboCount <= self._usedComboCount then
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:eventCallUsedSkill_TrySkillCombo(skillWrapper)
  local listNo = 0
  local skillNo = skillWrapper:getSkillNo()
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:changeStepSuggestCallBlackSpirit()
  if 1 == self._currentProgress then
    PaGlobal_TutorialUiManager:getUiBlackSpirit():showSuggestCallSpiritUi()
    PaGlobal_TutorialManager:setAllowCallBlackSpirit(true)
  end
end
function PaGlobal_TutorialPhase_BasicSkill_Combattant:eventCallAcceptBlackSpiritQuest(isAccept, questGroupNo, questId)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:eventCallAfterBlackSpiritDialogClose()
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:handleEventQuestUpdateNotify(isAccept, questNoRaw)
...
function PaGlobal_TutorialPhase_BasicSkill_Combattant:handleAfterAndPopFlush()
  if 1 == self._currentType and 3 == self._currentStep then
    self:eventCallAfterBlackSpiritDialogClose()
  end
end
function PaGlobal_TutorialPhase_BasicSkill_Combattant:handleEventSelfPlayerUsedSkill(skillWrapper)
