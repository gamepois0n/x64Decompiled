...
function ServantInfo_BeforOpenByActorKeyRaw(actorKeyRaw)
  ...
  elseif UI_VT.Type_SailingBoat == vehicleType or UI_VT.Type_PersonalBattleShip == vehicleType or UI_VT.Type_PersonTradeShip == vehicleType or UI_VT.Type_PersonalBoat == vehicleType then
...
function ServantInfo_Open()
  ...
  if nil ~= landVehicleWrapper and UI_VT.Type_RidableBabyElephant == landVehicleWrapper:getVehicleType() then
...
function ServantInfo_BabyElephantWarning(isShow)
  if not isShow then
    TooltipSimple_Hide()
    return
  end
