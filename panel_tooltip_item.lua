...
local isMaxEnchanterEnable = false
local normalTooltip = {
...
function Panel_Tooltip_Item_Show(itemStaticStatus, uiBase, isSSW, isItemWrapper, chattingLinkedItem, index, isItemMarket, invenSlotNo)
...
function Panel_Tooltip_Item_Show_GeneralNormal(slotNo, slotType, isOn, index)
  ...
  if slotType == "servant_inventory" then
    if slotNo >= getServantInventorySize(ServantInventory_GetActorKeyRawFromIndex(0)) then
      return
    end
    parent = true
  ...
  elseif not isEquipOn then
    ...
    if not itemSSW:isEquipable() then
      isEquipalbeItem, servantItem = showTooltip_Item(normalTooltip, itemWrapper, false, true, nil, nil, nil, nil, itemNamingStr)
    else
      isEquipalbeItem, servantItem = showTooltip_Item(normalTooltip, itemWrapper, false, true, nil, slotNo, nil, nil, itemNamingStr)
    end
  else
...
function Panel_Tooltip_Item_Show_General(slotNo, slotType, isOn, isNormal, index)
  ...
  if nil ~= slot._type and (UI_RewardType.RewardType_Exp == slot._type or UI_RewardType.RewardType_SkillExp == slot._type or UI_RewardType.RewardType_ProductExp == slot._type) then
    return false
  end
  return true
end
...
function Panel_Tooltip_Item_Set_Position(positionData, parent, inven)
  local itemShow = Panel_Tooltip_Item:GetShow()
  local equipItemShow = Panel_Tooltip_Item_equipped:GetShow()
  if not itemShow and not equipItemShow then
    return
...
function Panel_Tooltip_Item_ShowInfo(target, inputValue, isSSW, isItemWrapper, chattingLinkedItem, index, isNextEnchantInfo, invenSlotNo, itemNamingStr)
  ...
  if nil ~= balksCount and toInt64(0, 0) ~= balksCount and itemSSW:get():isCash() and (isExtractionCommon or isExtractionJapan) then
  ...
  if nil ~= cronsCount and toInt64(0, 0) ~= cronsCount and itemSSW:get():isCash() and isCronStone and (isExtractionJapan or isExtractionCommon) then
...
- ]] .. itemSSW:getEnchantDescription()
  ...
  if target.cronStoneEnchant:GetShow() then
    ...
    target.cronStoneGrade:SetPosY(TooltipYPos + 5)
    TooltipYPos = GetBottomPos(target.cronStoneGrade, true)
  ...
  if not itemSSW:get():isCash() then
    if itemSSW:isEquipable() and 0 < itemSSW:getRecoveryMaxEndurance() and dynamicMaxEndurance <= 50 then
